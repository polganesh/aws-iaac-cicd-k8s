# ToDO check if furter rules require for eks
resource "aws_iam_role" "demo-codepipeline" {
  name = local.role_name_pipeline
  tags = merge(
    {
      Name    = local.role_name_pipeline
...
data "aws_iam_policy_document" "demo-codepipeline-role-policy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${local.role_name_pipeline}",
    ]
  }
  statement {
    effect = "Allow"
  ...
  statement {
    ...
    # providing rights for all code commit repos and not selective
    resources = [
      "*",
    ]
    #    resources = [
...
resource "aws_iam_role_policy" "demo-codepipeline" {
  name   = local.policy_name_pipeline
